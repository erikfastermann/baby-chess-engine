...
fn king<S: Side>(side: &mut S, visitor: &mut impl Visitor, old_enemy: &PlayerBoard) -> bool {
    apply_moves(
        ...
        visitor,
        old_enemy,
        |player_board| player_board.king_mut(),
        Moves::king,
    )
}
...
fn knights<S: Side>(side: &mut S, visitor: &mut impl Visitor, old_enemy: &PlayerBoard) -> bool {
    apply_moves(
        ...
        visitor,
        old_enemy,
        |player_board| player_board.knights_mut(),
        Moves::knight,
    )
}
...
fn bishops<S: Side>(side: &mut S, visitor: &mut impl Visitor, old_enemy: &PlayerBoard) -> bool {
    apply_moves(
        ...
        visitor,
        old_enemy,
        |player_board| player_board.bishops_mut(),
        Moves::bishop,
    )
}
...
fn rooks<S: Side>(side: &mut S, visitor: &mut impl Visitor, old_enemy: &PlayerBoard) -> bool {
    apply_moves(
        ...
        visitor,
        old_enemy,
        |player_board| player_board.rooks_mut(),
        Moves::rook,
    )
}
...
fn queens<S: Side>(side: &mut S, visitor: &mut impl Visitor, old_enemy: &PlayerBoard) -> bool {
    apply_moves(
        ...
        visitor,
        old_enemy,
        |player_board| player_board.queens_mut(),
        Moves::queen,
    )
}
...
fn can_castle_right<S: Side>(side: &mut S) -> bool {
    ...
    for shift in 0..=2 {
        side.we_mut().king_mut().mov(king_starting_index, king_starting_index+shift);
        let check = side.enemy().has_check(side.we(), S::color().other());
        *side.we_mut().king_mut() = we_old_king;
        if check {
            return false;
        }
    }
...
fn castle_reset(
    ...
    *side.we_mut().king_mut() = we_old_king;
    *side.we_mut().rooks_mut() = we_old_rooks;
}
...
fn pawns<S: Side>(
    ...
    let cont = apply_moves(
        ...
        visitor,
        old_enemy,
        |player_board| player_board.pawns_mut(),
    ...
    let cont = apply_moves_with(
        ...
        S::pawn_promotion,
        |side, from, to| {
            side.we_mut().pawns_mut().clear(to);
...
fn en_passant_single<S: Side>(
    ...
    side.enemy_mut().pawns_mut().clear(captured);
    side.we_mut().pawns_mut().mov(from, to);
    let cont = visitor.visit(side, Move::Normal { from, to });
    side.we_mut().pawns_mut().mov(to, from);
    *side.enemy_mut() = *old_enemy;
    cont
}
